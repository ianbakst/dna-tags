...
class DoubleErrorCorrectionEncoder(Encoder):
    def __init__(self, message_length: Optional[int] = None, total_length: Optional[int] = None):
        if (message_length is None and total_length is None) or (
            message_length is not None and total_length is not None
        ):
        ...
        if message_length is not None:
            ...
            for i in range(2, 10):
                if message_length <= (2**i - i - 1):
                    self.parity = i
                    self.total_length = self.parity + message_length + 1
                    break
    ...
    def decode(self, tag: List[Base]) -> List[Base]:
        ...
        correction = [sum(get_from_list(tag, order=p)) % 4 for p in range(self.parity, 0, -1)][::-1]
        overall_parity = sum(tag) % 4
        set_corr = set(correction)
        if len(set_corr) > 2 or max(correction) != overall_parity:
            raise DecodeError("Two or more errors detected.")
        if max(correction) == 0:
            return tag[:self.total_length]
